...
import com.campus.platform.ui.viewmodel.profile.MyBoughtFilter
import com.campus.platform.ui.viewmodel.profile.MyBoughtViewModel

// ── Filter chip mapping ──────────────────────────────────────

private val boughtFilterLabels = listOf("全部", "待面交", "已完成")

private fun labelToFilterKey(index: Int): String = when (index) {
    0 -> MyBoughtFilter.ALL
    1 -> MyBoughtFilter.PENDING_MEETUP
    2 -> MyBoughtFilter.COMPLETED
    else -> MyBoughtFilter.ALL
}

private fun filterKeyToIndex(key: String): Int = when (key) {
    MyBoughtFilter.ALL -> 0
    MyBoughtFilter.PENDING_MEETUP -> 1
    MyBoughtFilter.COMPLETED -> 2
    else -> 0
}
...
@Composable
fun MyBoughtScreen(
    viewModel: MyBoughtViewModel = hiltViewModel(),
                ...
                when {
                    ...
                    isRealError -> {
                        RunnerEmptyState(
                            ...
                            actionLabel = "重试",
                            onAction = { viewModel.loadBoughtOrders() },
                            modifier = Modifier.align(Alignment.Center),
                        )
                    }
                            ...
                            LazyColumn(
                                ...
                                items(
                                    ...
                                    MarketOrderCard(
                                        ...
                                        counterparty = "卖家：${shortId(order.sellerId)}",
